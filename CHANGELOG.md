...
- **`/rs-update ci` mode** — opt-in via the `ci` arg or `ROOTSPEC_CI=1` (also auto-detected via `CI=true`). No confirmation prompts, recommended path per violation, **transactional**: pre-flight every planned action before writing any files, abort the whole run on any failure. Structured key=value report at end (`RESULT`/`APPLIED`/`SKIPPED`/`ABORTED_ON`/`EXIT`) for CI scripts to grep. Fixes the three CI failure modes from 7.6.1 runs (hung-on-prompt, half-migrated brownfield, broken-stub scaffold).
- **`dev_cmd_recursive` violation in `gap-analysis.sh`** — detects the dev.sh/preview.sh template footgun: `DEV_CMD="npm run dev"` (or any equivalent that recurses through the wrapper) AND package.json routes through the wrapper. Both must be true for the violation to fire — projects that bypass the wrapper from package.json don't recurse, but that's its own bug (defeats the wrapper's port handling and single-instance guarantees), and the new framework rule pulls them through `/rs-update` reconciliation when they next upgrade. New Step 5 reconciliation in `rs-update/SKILL.md` walks the developer through detection + substitution.

### Changed

- **`scripts/dev.sh` and `scripts/preview.sh` ship with empty `DEV_CMD`/`PREVIEW_CMD` and a hard-fail recursion guard.** First invocation before `/rs-impl` populates them exits 1 with an actionable message. The guard rejects empty values, `npm run dev` / `yarn dev` / `pnpm dev` / `bun run dev`, and `./scripts/dev.sh` (and equivalents for preview). Closes the 7.6.1 footgun: the templates shipped with `DEV_CMD="npm run dev"` and `bootstrap-init.sh` wired package.json's `dev` script to `./scripts/dev.sh start`. Both contracts together = infinite recursion. The reason it didn't loop in scaffold/brownfield is that those projects' package.json `dev` scripts had been hand-overwritten to bypass the wrapper — masking the bug while defeating the wrapper. Now: package.json always routes through the wrapper, the wrapper always calls the framework binary directly, and `/rs-impl` populates the binary command from `detect-stack.sh`.
- **`bootstrap-init.sh`** captures the project's pre-bootstrap `package.json` `dev`/`preview` scripts into `.rootspec.json` `prerequisites.detected.devCmd` / `.previewCmd` BEFORE overwriting them with the wrapper-routing versions. `detect-stack.sh` reads these first so brownfield projects' custom flags survive the bootstrap.
- **`framework-rules.md`** — new `## Dev Server Wrapper` section: package.json must route through `./scripts/dev.sh` / `./scripts/preview.sh`; the wrappers must call the framework binary directly (no `npm run *`, no self-recursion). Two rules, framework-agnostic.
- **`rs-impl/SKILL.md` Step 2c** — sources `detect-stack.sh`, populates `DEV_CMD`/`PREVIEW_CMD` in the wrappers, records the choice in `CONVENTIONS/technical.md` → Dev Server. Future `/rs-impl` runs read the conventions section as authority — only re-run detection if the section is missing.
- **`rs-init/SKILL.md` Step 3** — drops the "edit `DEV_CMD` manually" instruction; explains that `dev.sh` ships intentionally non-functional and `/rs-impl` populates it from the detected stack (or captured-original).
- **`rs-update/SKILL.md`** — entire skill restructured around the **Mode** distinction (Interactive vs CI). Step 3 has a transactional pre-flight phase that runs in CI mode; Step 5 has a CI policy summary table and per-violation `**CI:**` notes for every existing violation; Step 6 has a structured key=value report format for CI consumers; Focus section documents the `ci` arg.
- **`scaffold-cypress.sh`** consumes `detect-stack.sh` for the baseUrl port (single source of truth); the inline framework-detection block (astro/vite/next/nuxt) is gone.
...
- **Review gate resilience**: Initialize `review-status.json` in the discovery bash command (Turn 1) instead of relying on the agent to write it later. Prevents empty gate result when review hits max_turns before writing.
...
- **Review skill — user perspective + turn efficiency**: Rewritten to review from rendered HTML + screenshots instead of source files. Broken links are only blockers if provably wrong (404, placeholder URL), not just "not in SEED.md verbatim." Section-based batching (group stories by YAML file) with incremental `review-status.json` writes for cut-off resilience. Turn budget guidance injected by orchestrator.
...
- **rs-impl package.json hard rule**: Elevated package.json constraint to preamble-level hard rule and added to Scope section as CANNOT. Previous soft guidance (v7.0.9) was ignored by the agent.
...
- **00.SPEC_FRAMEWORK.md not available post-install**: Moved canonical file into `rs-shared/`, root file is now a symlink
...
- Symlink auto-detection for Cypress UI filtering (`autoFilter` map)
- Release script (`scripts/release.sh`) for automated version management
...
- CLI `--version` flag now shows correct version (was showing 4.1.0 in 4.2.0)
- Build script now makes bin file executable to fix npm warning
