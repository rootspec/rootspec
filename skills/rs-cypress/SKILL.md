...
Bundled templates are in `templates/`. These are the canonical source — copy them, don't generate from scratch.
...
2. Copy the bundled templates to the project:
   - Read `templates/cypress.config.ts` → write to `cypress.config.ts`
   - Read `templates/cypress/support/e2e.ts` → write to `cypress/support/e2e.ts`
   - Read `templates/cypress/support/steps.ts` → write to `cypress/support/steps.ts`
   - Read `templates/cypress/support/schema.ts` → write to `cypress/support/schema.ts`
   - Read `templates/cypress/e2e/example.cy.ts` → write to `cypress/e2e/example.cy.ts`
...
2. Read the corresponding bundled template from `templates/`
...
- "Extend the DSL by adding steps to `cypress/support/steps.ts` and `cypress/support/schema.ts` — see `../rs-shared/fragments/l5-test-dsl.md` for the extension pattern"
