...
export default defineConfig({
  e2e: {
    ...
    setupNodeEvents(on, config) {
      ...
      // Test ledger: tracks pass/fail history per acceptance criterion
      // Writes to test-ledger.json after each run
      const { registerLedgerReporter } = require('./scripts/reporters/cypress-ledger-reporter');
      registerLedgerReporter(on);
