...
| `viewport_missing` | None — file edits are idempotent; never deletes test files | Pass |
...
| `viewport_missing` | Auto-extend `cypress/support/schema.ts` and `steps.ts` with `setViewport` + geometry assertions (idempotent); auto-add Test Viewports section to `CONVENTIONS/technical.md` if missing. Never deletes existing `.cy.ts` files; emits `WARN=cypress_e2e_needs_regen` so the developer knows to regenerate. |
...
The project has at least one user story tagged `@journey: MOBILE_*` or `@journey: TABLET_*`, generated `cypress/e2e/*.cy.ts` files exist, and none of those test files contain `cy.viewport(`. The pre-7.8.0 generator had no viewport awareness — those tests run at Cypress's 1000×660 default regardless of the journey. A "no horizontal scroll at 375px" criterion passes vacuously against a desktop-width viewport.

Apply phase (the same on both interactive and CI):

1. **Extend `cypress/support/schema.ts`** — add `setViewport` to the Step union (and `shouldHaveNoOverflowX`/`shouldFitViewport` if missing). Idempotent; skip lines already present.
2. **Extend `cypress/support/steps.ts`** — add `else if ('setViewport' in s) cy.viewport(s.setViewport.width, s.setViewport.height);` to `runSetupSteps`; add the geometry-assertion branches to `runAssertionSteps`. Idempotent.
3. **Add Test Viewports to `CONVENTIONS/technical.md`** — if the section is missing, append it under Testing using the framework defaults from `../rs-shared/viewport-defaults.json`:
...
4. **Tell the developer** that existing `cypress/e2e/*.cy.ts` files were generated before viewport awareness and need regeneration:

   > Existing tests for mobile/tablet-journey stories were generated before viewport awareness. Delete the affected `cypress/e2e/*.cy.ts` files and re-run `/rs-impl` to regenerate them with `setViewport` injection. (We won't delete user-owned test files automatically.)

**Interactive:** confirm before each file edit; offer to regenerate the test files at the end.
**CI:** auto-apply steps 1–3 (idempotent file edits, framework-owned mechanism), skip the regenerate step (CI doesn't delete user files), include `WARN=cypress_e2e_needs_regen` in the structured report so the next developer-led run picks up.
...
When `viewport_missing` is in `APPLIED`, also emit `WARN=cypress_e2e_needs_regen` so CI scripts see that the test files need to be deleted + regenerated by the next developer-led `/rs-impl`. Multiple warnings are comma-separated.
