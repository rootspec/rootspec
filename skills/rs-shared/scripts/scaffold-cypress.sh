...
write_if_missing "cypress/support/app-ready.ts" "// Implementation lives below. The signal MUST observe any deferred-execution
// boundaries the app uses (client directives, lazy/Suspense, dynamic imports).
// Document the chosen mechanism in rootspec/CONVENTIONS/technical.md → App Readiness.
...
Cypress.Commands.add('appReady', () => {
  throw new Error(
    ...
    'rootspec/CONVENTIONS/technical.md → App Readiness for project-specific guidance.'
  );
});
