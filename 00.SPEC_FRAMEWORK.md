...
|-------|------|---------|--------------|----------|------------|
| **1: Foundational Philosophy** | 📘 | WHY & WHAT EXPERIENCE | "What problem must we solve? What should users feel?" | Founders, designers, domain experts | External only |
| **2: Stable Truths** | ⚙️ | Design strategies & commitments | "What approach will we take?" | System designers, product strategists | L1 + External |
| **3: Interaction Architecture** | 🔄 | HOW users and product interact | "What's the behavioral pattern?" | Interaction designers, developers | L1-2 + External |
| **4: Systems** | 🧩 | Implementation architecture | "How do we build this?" | Developers, AI systems, implementers | L1-3 + Sibling L4 + External |
| **5: Implementation** | ⚖️ | Validation & numeric tuning | "Does it work? What values?" | PMs, QA, data scientists | All levels + External |
...
**Design Pillars:** Short, emotional phrases (1 sentence max) that capture the essence of the user experience. These serve as decision filters—features must support at least one pillar. Focus on how users will FEEL, not what they will do.
...
#### ⚙️ **Level 2: Stable Truths** — WHAT Strategy
...
**Suggested Structure:**
- `SYSTEMS_OVERVIEW.md` — System interconnections
- `[DOMAIN]_SYSTEM.md` — One per major subsystem
...
**Example User Story:**
```markdown
As a user completing a task, I want to see points awarded immediately (<100ms), so that I experience instant positive feedback.

Acceptance criteria:
- Points display updates within 100ms of task completion
- Visual feedback animation plays during update
- System references: TASK_SYSTEM.md, REWARD_SYSTEM.md
```
...
- **Foundational documents remain stable** — Philosophy doesn't change when you adjust numbers
- **Changes flow downward** — Implementation evolves without corrupting principles
- **Architectural integrity** — High-level decisions guide low-level details, not vice versa
- **Separation of concerns** — What vs Why vs How vs How Much live at appropriate levels
...
```
/
├── 00.SPEC_FRAMEWORK.md               # This document
├── 01.FOUNDATIONAL_PHILOSOPHY.md      # Why the product exists & core experiences
├── 02.STABLE_TRUTHS.md                # Design philosophy and commitments
├── 03.INTERACTION_ARCHITECTURE.md     # How users and product interact
├── 04.SYSTEMS/                        # Implementation architecture
│   ├── SYSTEMS_OVERVIEW.md           # How systems interconnect
│   ├── [SYSTEM_A].md                 # First major subsystem
│   ├── [SYSTEM_B].md                 # Second major subsystem
│   └── [SYSTEM_N].md                 # Additional subsystems
└── 05.IMPLEMENTATION/                # Validation and tuning
    ├── USER_STORIES/                 # User perspective validation
    │   ├── USER_STORIES_OVERVIEW.md  # Organization and index
    │   ├── by_priority/              # Organized by development phase
    │   │   ├── MVP.md                # Core functionality stories
    │   │   ├── SECONDARY.md          # Enhanced experience stories
    │   │   └── ADVANCED.md           # Future feature stories
    │   ├── by_journey/               # Organized by user flow
    │   │   ├── [JOURNEY_A].md        # First user journey
    │   │   └── [JOURNEY_B].md        # Second user journey
    │   └── by_system/                # Organized by system
    │       ├── [SYSTEM_A]_STORIES.md  # Stories for System A
    │       └── [SYSTEM_B]_STORIES.md  # Stories for System B
    └── FINE_TUNING/                  # Numeric balancing (comment-annotated YAML)
        ├── [SYSTEM_A].yml            # Parameters for System A
        ├── [SYSTEM_B].yml            # Parameters for System B
        └── [PARAMETERS_N].yml        # Additional parameter sets
```
...
1. **Determine the correct level** — Ask: "Is this about WHY, WHAT strategy, HOW it works conceptually, HOW it's implemented, or HOW MUCH?"
2. **Check reference constraints** — Ensure you only reference appropriate levels
3. **Use placeholders** — In Levels 1-4, use descriptive placeholders like `[short duration]` instead of `200ms`
4. **Maintain abstraction** — Each level should be complete without depending on lower levels

### Organizing User Stories at Level 5

User stories belong at **Level 5** (Implementation), where they validate system implementations from the user's perspective. A user story expresses the **intent and value** of a feature from the user's perspective, not the implementation. Each story should describe who the user is, what they need to do, and why it matters, using natural, goal-oriented language.
...
- They provide acceptance criteria for validating implementations

**Core principles:**

- Stories focus on **user outcomes** rather than technical details
- Stories tell **what** and **why**, leaving the **how** for engineers to decide
- Acceptance criteria define **observable behaviors** and success conditions
- Stories should be specific enough to implement unambiguously
...
User stories should be organized in multiple ways to support different perspectives:

1. **by_priority/** — Groups stories by development phase (MVP, Secondary, Advanced)

   - Helps prioritize implementation efforts
   - Aligns with product roadmap and release planning

2. **by_journey/** — Groups stories by user flow (e.g., Onboarding, Daily Usage, Reflection)

   - Validates complete user experiences end-to-end
   - Ensures coherent user journeys across systems

3. **by_system/** — Groups stories by primary system referenced
   - Links validation back to system architecture
   - Helps verify each system delivers user value

Stories can (and should) appear in multiple organizational views by reference or duplication, depending on your workflow preferences.

**Format:**

```
As a [user type/state], I want to [action/goal], so that [outcome/value].
...
System references: [SYSTEM_A].md, [SYSTEM_B].md
```

**Examples:**

**1. Core functionality (everyday user need)**
...
**2. Cross-system integration (spanning multiple systems)**
...
>
> - Points display updates within 100ms of task completion
> - Visual feedback animation plays during the update
> - Point total reflects the correct calculated value based on task properties
> - Occasional bonus rewards appear (~7% of completions)
>
> **System references:** TASK_SYSTEM.md, REWARD_SYSTEM.md, VIEW_SYSTEM.md

**3. Trust and control (safety or reliability feature)**

> As a careful user, I want to confirm before permanently deleting a task so that I don't lose important items by mistake.
>
> **Acceptance criteria:**
>
> - Deletion triggers a confirmation prompt.
> - Confirming deletes the task permanently; canceling does nothing.
> - Optionally, the prompt can be disabled in Settings for power users.
>
> **System references:** TASK_SYSTEM.md

Each story defines **who**, **what**, and **why** (the intent and value), while the acceptance criteria define the measurable behaviors without prescribing the implementation. System references link the story back to relevant architectural specifications.
...
1. **Values are first-class** — The YAML structure contains only deployable configuration values
2. **Metadata in comments** — All annotations, rationale, and context use comment syntax
3. **Machine-parseable** — Strip comments to get clean config; parse comments for documentation
4. **Human-readable** — Rich context without cluttering the data structure
5. **Traceable** — Every value links back to spec levels and decisions
...
- **Clean YAML** — Strip comments → deployable config
- **Rich documentation** — All context inline with values
- **Traceability** — Every value traces to spec decisions
- **Version control** — Git diffs show values + context changes
- **Validation** — `@constraints` enable automated validation
- **Tooling-friendly** — Parse `@annotations` for automation

**Tooling Recommendations:**

1. **Config Generator** — Strip all comments to produce clean deployment configs
2. **Documentation Generator** — Extract `@rationale` and `@reference` to create decision logs
3. **Validator** — Parse `@constraints` to validate values
4. **Traceability Tool** — Follow `@spec_source` back to architectural docs
5. **Change Impact Analyzer** — Use `@impact` to assess change ripple effects

### When Modifying Documents

1. **Start at the appropriate level** — If changing philosophy, start at Level 1; if tuning numbers, work in Level 5
2. **Propagate changes downward** — Update lower levels to align with changes in higher levels
3. **Never propagate upward** — Implementation details should never require changes to philosophy
4. **Verify references** — Ensure no new violations of the hierarchy rules

### When Reviewing Documents

- **Check for leaky abstractions** — Does a high-level doc reference implementation details?
- **Verify single source of truth** — Is each concept defined at exactly one level?
- **Confirm stability** — Could you completely reimplement Level 4 without touching Levels 1-3?
...
1. **Keep the 5-level structure** — The hierarchy has proven effective across diverse domains
2. **Rename Level 3 if needed** — "Interaction Architecture" works for many products, but you might use "Behavioral Architecture," "Information Architecture," "Flow Architecture," etc.
3. **Customize Level 4 subsystems** — Your systems will be specific to your domain
4. **Organize Level 5 appropriately** — Use USER_STORIES/ for validation and FINE_TUNING/ for parameters, or adapt the structure to your implementation needs
5. **Maintain reference rules** — This is the core constraint that makes the framework valuable
6. **Add a project-specific guide** — Consider creating a `CLAUDE.md` or similar to guide AI assistants on your specific terminology and conventions
...
1. **Always start at Level 1** — Resist jumping to implementation details; understand the "why" first
2. **One level at a time** — Complete each level before descending to the next
3. **Ask "why" more than "what"** — Understanding intent prevents misaligned implementations
4. **Use concrete examples** — Request specific scenarios and user stories, not abstractions
5. **Validate hierarchy** — Continuously check that levels don't reference lower levels
6. **Challenge assumptions** — Question unstated dependencies and hidden implementation details
7. **Identify anti-patterns** — Help users avoid common specification mistakes
...
1. **"What problem does this product solve, and why must it exist?"** — Focus on user pain and transformation, not features (Mission)
2. **"What 3-5 core experiences or emotions should users feel?"** — These become your Design Pillars
3. **"For each pillar, what specific feeling are you creating?"** — Joy? Empowered? Calm? Confident? Be specific.
4. **"What principles would you never violate, even if it cost features or users?"** — Identify inviolable constraints
5. **"Who is this NOT for? What will this product NOT do?"** — Define boundaries through exclusion
6. **"If this product succeeds, how will users' lives be different in 6 months?"** — Validate long-term impact (North-star)
...
1. **"What design philosophy or framework guides your approach?"** — Identify existing paradigms or create new ones
2. **"What are you optimizing for, and what trade-offs will you accept?"** — Surface competing values
3. **"What patterns from other domains apply here?"** — Game design? Education? Social systems?
4. **"What existing approaches are you rejecting, and why?"** — Learn from what NOT to do
5. **"How do you define success for this product?"** — Metrics, user behaviors, outcomes

#### **Level 3: Interaction Architecture**

1. **"Walk me through a complete user journey from start to finish"** — Get concrete flow, not abstract description
2. **"What triggers each interaction, and what feedback does the user receive?"** — Map the complete loop
3. **"Are there different scales of interaction? Immediate? Session-level? Cross-session?"** — Identify temporal architecture
4. **"What happens when things fail or users skip steps?"** — Explore edge cases early
5. **"How do different systems coordinate to create coherent experiences?"** — Understand cross-system orchestration

#### **Level 4: Systems**

1. **"What are the major subsystems, and what is each responsible for?"** — Establish boundaries
2. **"What data does each system manage? What are the key entities?"** — Define information architecture
3. **"What rules govern state transitions and system behavior?"** — Extract business logic
4. **"How do systems interact? What do they expose to each other?"** — Map interfaces and dependencies
5. **"What values are calculated, and from what inputs?"** — Identify derived fields and formulas

#### **Level 5: User Stories**

1. **"As a user, what would you want to accomplish in the first 5 minutes? First day? First week?"** — Extract temporal user needs
2. **"For each feature, what observable behaviors would confirm it's working?"** — Define acceptance criteria
3. **"Which features are absolutely essential (MVP) vs. nice-to-have vs. future?"** — Establish priority
4. **"What are the complete user journeys from entry to exit?"** — Map end-to-end flows
5. **"How would you know if the system is delivering the intended experience?"** — Validate alignment with Level 1

#### **Level 5: Fine Tuning**

1. **"What timing values matter? Response delays? Animation durations?"** — Identify temporal parameters
2. **"What numeric thresholds determine state changes?"** — Find decision boundaries
3. **"What scaling factors affect calculations?"** — Identify growth curves and multipliers
4. **"What are the rationales for these specific numbers?"** — Document reasoning for future tuning
5. **"How might these values change based on testing or feedback?"** — Plan for iteration
...
| **Special Requirements** | — | — | — | — | L5 Fine Tuning: Parameters organized logically; References to source systems |
...
- **Philosophical stability** — Core values don't change with implementation details
- **Implementation flexibility** — Completely refactor systems without changing principles
- **Clear decision boundaries** — Know where each type of decision belongs
- **Reduced cognitive load** — Each document has a clear scope and audience
- **Easier onboarding** — New team members can read from Level 1 downward
- **Safer AI assistance** — AI systems can understand and respect architectural constraints
- **Testable integrity** — Can automatically validate reference rules
