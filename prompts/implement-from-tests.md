# Implement Application from YAML Tests

Complete prompt for implementing an application iteratively using the YAML user stories generated from your specification.
...
- [ ] Generated YAML user stories in `05.IMPLEMENTATION/USER_STORIES/`
- [ ] Cypress test framework installed (see `docs/CYPRESS_SETUP.md`)
- [ ] Understanding of test DSL core steps
...
- Implementing features to pass existing YAML user story tests
- Working test-first with specs already defined (spec-driven development)
- Iteratively building application guided by acceptance criteria
...
```
I have a complete specification following Hierarchical Specification Framework v3.0.0,
...
- AI scans all YAML user stories
- Lists all DSL steps needed
- Identifies global setup requirements
- Creates implementation roadmap

### Phase 2: Global Setup (30-60 minutes)
- Implement auth pattern (loginAs)
- Implement database reset
- Implement seed data pattern
- Test that global setup works

### Phase 3: Iteration (varies by project)
- **MVP tests:** 1-4 weeks for typical SaaS MVP
- **POST_MVP tests:** 2-4 weeks additional
- **Per test:** 15 minutes - 2 hours depending on complexity

**Total time for MVP:** 2-6 weeks of focused development
...
- **Start with MVP** - Don't try to implement everything at once
- **One test at a time** - Focus on making one acceptance criteria pass before moving on
- **Follow the spec** - Reference Level 4 SYSTEMS for architecture guidance
- **Use data-test attributes** - Exactly match selectors in YAML
- **Commit frequently** - After each passing test
- **Run tests often** - Get quick feedback

### Don't

- **Skip global setup** - You'll regret it when tests become flaky
- **Hardcode values** - Use FINE_TUNING parameters
- **Batch too much work** - One failing test at a time
- **Ignore test failures** - Debug immediately, don't accumulate tech debt
- **Change YAML tests** - If test seems wrong, check spec first, then discuss
- **Violate spec architecture** - Level 4 SYSTEMS define your structure

---

## Integration with Other Prompts

- **Before this prompt:** Use `initialize-spec.md` or `adopt-framework-existing.md`
- **During iteration:** Use `validate-spec.md` if you find spec issues
- **After MVP complete:** Use `add-feature.md` to expand specification
- **For code review:** Use `review-feature.md` to validate implementation
...
- Application architecture matches Level 4 SYSTEMS
- User experience matches Level 3 interaction patterns

**Test Infrastructure:**
- Robust global setup (auth, DB reset, seeds)
- Extended DSL for domain-specific operations
...
2. **Run POST_MVP tests** - Continue iteration workflow
3. **Update spec based on learnings** - Use `add-feature.md`
4. **Refactor with confidence** - Tests prevent regressions
5. **Scale the team** - Spec enables new developers to onboard quickly
