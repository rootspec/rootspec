...
export async function cypressCommand() {
    ...
    await fs.copy(path.join(templatesSrc, 'cypress.config.ts'), path.join(cwd, 'cypress.config.ts'));
    console.log(chalk.green('   ✓ cypress.config.ts'));
    // Copy cypress/ directory
    await fs.copy(path.join(templatesSrc, 'cypress'), path.join(cwd, 'cypress'));
    ...
    if (!await fs.pathExists(userStoriesDir)) {
        ...
        await fs.copy(path.join(templatesSrc, 'USER_STORIES'), path.join(cwd, 'templates', 'USER_STORIES'));
